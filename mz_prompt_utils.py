...
class Utils:
    ...
    def get_models_path():
        models_path = os.path.join(folder_paths.models_dir, "minus_zone_models")
    ...
    def mask_threshold(interested_mask):
        ...
        ret,thresh1 = cv2.threshold(mask_image_cv2,127,255,cv2.THRESH_BINARY)
    ...
    def latent_upscale_by(samples, scale_by):
        ...
        s["samples"] = comfy.utils.common_upscale(samples["samples"], width, height, "nearest-exact", "disabled")
        return s
        ...
        def get_total_existing_size():
            ...
            for f in fs:
                if f.startswith("block_"):
                    existing_size += os.path.getsize(os.path.join(cache_dir, f)) 
        ...
        if total_size != 0 and total_existing_size != total_size:
            ...
            with tqdm(total=total_size, initial=total_existing_size, unit="B", unit_scale=True) as progress_bar:
                ...
                for i in range(threads): 
                    ...
                    t = threading.Thread(target=download_file_thread, args=(response, cache_filepath))
            ...
            if total_size != 0 and total_size != get_total_existing_size():
                # 文件损坏 
                raise RuntimeError(f"Download failed: {total_size} != {get_total_existing_size()}")
            
        if os.path.exists(filepath):
            shutil.move(filepath, filepath + ".old." + time.strftime("%Y%m%d%H%M%S"))
    ...
    def hf_download_model(url, only_get_path=False):
        ...
        if url.startswith("https://huggingface.co/") or url.startswith("https://hf-mirror.com/"):
            base_model_path = os.path.abspath(os.path.join(Utils.get_models_path(), "transformers_models"))
            ...
            if os.path.exists(save_path) is False:
                if only_get_path:
                    return None
                os.makedirs(os.path.join(base_model_path, zone_path), exist_ok=True)
                Utils.download_file(url, save_path)

            Utils.print_log(f"File {save_path} => {os.path.getsize(save_path)} ") 
            ...
            base_model_path = os.path.abspath(os.path.join(Utils.get_models_path(), f"{host}_models"))
            ...
            save_path = os.path.join(base_model_path, f"{file_name_no_ext}.{md5_hash}{file_ext}")
    ...
    def modelscope_download_model(model_type, model_name, only_get_path=False):
        ...
        save_path = os.path.abspath(os.path.join(Utils.get_models_path(), output))
